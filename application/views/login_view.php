					...
					$.ajax({
						...
						success : function(result){
							...
							else
							{
								...
								$.ajax({
										url			: base_url + "pemda/update_last_login",	
										type		: 'POST',
										data		:  $("#formLogin").serialize(),
										dataType	: 'JSON',
										success : function(result){
			...
			<div class = "panel box-shadow">
				<div class = "panel-body center-block">
					<div class = "pages-header text-center">
						<div class = "pages-box-icon">
							<!--<i class = "zmdi zmdi-account-o"></i>!-->
							<div align = "center" class = "img-responsive center-block" >
								<?php
									error_reporting(E_ALL & ~E_NOTICE);
									$user_id	= $this->session->userdata('user_id');									
									$menu		= $this->db->query("SELECT col_value FROM m_setting WHERE col_name = 'logo'");
									$data   =   array();
									foreach($menu->result_array() as $row){
										//echo $row['nama_menu'];	
								?>		
									<img src = "<?php echo base_url().'/assets/img/'.$row['col_value']; ?>" style="width: 80px;height: 80px;" >
								<?php
									}
								?>
							</div>
					...
					<fieldset>
						<div class = "form-group"><input class = "form-control" placeholder = "Username" name = "username" id = "username" type = "text" required autofocus></div>
						<div class = "form-group"><input class = "form-control" placeholder = "Password" name = "password" id = "password" type = "password" value = "" required></div>
						...
						<input class = "btn m-indigo btn-success btn-block" id = "btnLogin" value = "Login" type = "submit">
